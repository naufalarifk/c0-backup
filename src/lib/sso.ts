...
import type { Account, BetterAuthPlugin, OAuth2Tokens, Session, User } from 'better-auth';
    ...
    endpoints: {
      ...
      registerSSOProvider: createAuthEndpoint(
        ...
        async ctx => {
          ...
          const provider = await ctx.context.adapter.create<Record<string, any>, SSOProvider>({
            model: 'ssoProvider',
            data: {
              issuer: body.issuer,
              domain: body.domain,
              oidcConfig: body.oidcConfig
                ? JSON.stringify({
                    ...
                    overrideUserInfo:
                      ctx.body.overrideUserInfo || options?.defaultOverrideUserInfo || false,
                  })
                : null,
          ...
          return ctx.json({
            ...provider,
            oidcConfig: JSON.parse(provider.oidcConfig as unknown as string) as OIDCConfig,
            samlConfig: JSON.parse(provider.samlConfig as unknown as string) as SAMLConfig,
        ...
        async ctx => {
          ...
          if (provider.oidcConfig && body.providerType !== 'saml') {
            ...
            const authorizationURL = await createAuthorizationURL({
              ...
              scopes: ctx.body.scopes || ['openid', 'email', 'profile', 'offline_access'],
          ...
          if (provider.samlConfig) {
            const parsedSamlConfig = JSON.parse(provider.samlConfig as unknown as string);
            const sp = saml.ServiceProvider({
              metadata: parsedSamlConfig.spMetadata.metadata,
              allowCreate: true,
            });
            ...
            const loginRequest = sp.createLoginRequest(idp, 'redirect') as BindingContext & {
      ...
      callbackSSOSAML: createAuthEndpoint(
        ...
        async ctx => {
          ...
          if (existingUser) {
            ...
            if (!accounts) {
              ...
              if (!isTrustedProvider) {
                throw ctx.redirect(`${parsedSamlConfig.callbackUrl}?error=account_not_found`);
              }
          ...
          const session: Session = await ctx.context.internalAdapter.createSession(user.id, ctx);
