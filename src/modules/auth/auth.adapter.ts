    ...
    adapter: () => ({
      create({ model, data, select, ...others }) {
        ...
        if (model === 'user') {
          ...
          return userRepo.betterAuthCreateVerification(data) as any;
        } else if (model === 'twoFactor') {
          return userRepo.betterAuthCreateTwoFactor(data) as any;
        } else {
      ...
      update({ model, update, where, ...others }) {
        ...
        if (model === 'user') {
          return userRepo.betterAuthUpdateUser(where, update) as any;
        } else if (model === 'session') {
          return userRepo.betterAuthUpdateSession(where, update) as any;
        } else if (model === 'account') {
          return userRepo.betterAuthUpdateAccount(where, update) as any;
        } else if (model === 'verification') {
          return userRepo.betterAuthUpdateVerification(where, update) as any;
        } else if (model === 'twoFactor') {
          return userRepo.betterAuthUpdateTwoFactor(where, update) as any;
        } else {
      ...
      async delete({ model, where, ...others }) {
        ...
        try {
          if (model === 'user') {
            ...
            return deleted as any; // Return deleted record or null if not found
          } else if (model === 'account') {
            return userRepo.betterAuthDeleteAccount(where) as any;
          } else if (model === 'session') {
            return userRepo.betterAuthDeleteSession(where) as any;
          } else if (model === 'verification') {
            return userRepo.betterAuthDeleteVerification(where) as any;
          } else if (model === 'twoFactor') {
            return userRepo.betterAuthDeleteTwoFactor(where) as any;
          } else {
      ...
      async deleteMany({ model, where, ...others }) {
        ...
        try {
          if (model === 'user') {
            // Field mapping handled in repository methods
            return userRepo.betterAuthDeleteManyUsers(where) as any;
          } else if (model === 'account') {
      ...
      async findOne({ model, where, select, ...others }) {
        ...
        if (model === 'user') {
          // Field mapping handled in repository methods
          const user = (await userRepo.betterAuthFindOneUser(where)) as any;
          if (!user) return null;
      ...
      findMany({ model, where, limit, offset, sortBy, ...others }) {
        ...
        if (model === 'user') {
          ...
          return userRepo.betterAuthFindManyTwoFactor(where, limit, offset, sortBy) as any;
        } else {
