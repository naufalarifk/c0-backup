...
import { DatabaseCredentials, VaultAuth, VaultHealthStatus } from './vault-cryptography.dto';

@Injectable()
export class VaultCryptographyService
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(private appCofig: AppConfigService) {
    super();
  }
  ...
  private async initializeVault() {
    try {
      const vaultAddr = this.appCofig.cryptographyConfig.vaultAddress;
      const vaultToken = this.appCofig.cryptographyConfig.vaultToken;
  ...
  private async authenticateWithAppRole(): Promise<void> {
    const roleId = this.appCofig.cryptographyConfig.vaultRoleId;
    const secretId = this.appCofig.cryptographyConfig.vaultSecretId;
  ...
  async encrypt(keyName: string, plaintext: string, context?: string): Promise<EncryptionResult> {
    ...
    try {
      ...
      return { ciphertext: response.data.ciphertext, keyId: keyName };
    } catch (error) {
  ...
  async decrypt(keyName: string, ciphertext: string, context?: string): Promise<DecryptionResult> {
    ...
    try {
      ...
      return { plaintext, keyId: keyName };
    } catch (error) {
